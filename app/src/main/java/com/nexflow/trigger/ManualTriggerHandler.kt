...
@Singleton
class ManualTriggerHandler @Inject constructor() : TriggerHandler {

    override val supportedType = TriggerType.MANUAL

    private val _events = MutableSharedFlow<TriggerEvent>(extraBufferCapacity = 16)

    /** Called by FlowEngine.runNow() — bypasses this flow, so not typically needed here. */
    fun fire(trigger: Trigger) {
        _events.tryEmit(TriggerEvent(trigger.id, ""))
    }

    override fun observe(trigger: Trigger): Flow<TriggerEvent> =
        _events.filter { it.triggerId == trigger.id }
}
